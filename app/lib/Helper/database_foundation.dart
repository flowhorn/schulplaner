...
class DataDocumentPackage<T> {
  ...
  final List<StreamController<T?>> _list_streamcontroller = [];
  final List<StreamController<T?>> _list_streamcontroller_once = [];
...
class DataCombinedPackage<T> {
  final List<StreamController<Map<String, T>>> _list_streamcontroller = [];
  final List<StreamController<Map<String, T>>> _list_streamcontroller_once = [];
...
class DataCollectionPackage<T> {
  ...
  final List<StreamController<List<T>?>> _list_streamcontroller = [];
  final List<StreamController<List<T>?>> _list_streamcontroller_once = [];
  ...
  Stream<T?> getItemFilteredStream(ItemFilter<T> filter) {
    ...
    return newcontroller.stream
        .map((streamData) => streamData?.firstWhere(filter));
  }
  ...
  void _initiate() {
    ...
    _listener = reference.snapshots().listen((querySnapshot) {
      ...
      data = dataObjects;
      for (final controller in _list_streamcontroller) {
        controller.add(data);
      }
      for (final controller in _list_streamcontroller_once) {
        controller.add(data);
      }
      _list_streamcontroller_once.clear();
    });
